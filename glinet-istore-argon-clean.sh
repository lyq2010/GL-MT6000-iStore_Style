...
green "==> [2/5] 安装 iStore（官方 istore.linkease.com）"
...
[ -n "$IPK" ] || { red "无法获取 iStore 包列表，检查网络是否能访问 istore.linkease.com"; exit 1; }
...
/tmp/is-opkg opkg install --force-reinstall luci-app-store || { red "iStore 安装失败"; exit 1; }
...
$ISOPKG install luci-compat luci-lib-ipkg >/dev/null 2>&1
...
ARGONCFG_URL="https://github.com/jerrykuku/luci-app-argon-config/releases/download/v0.9/luci-app-argon-config_0.9_all.ipk"
...
if $FCURL -o /tmp/luci-theme-argon.ipk "$ARGON_URL" && \
   $FCURL -o /tmp/luci-app-argon-config.ipk "$ARGONCFG_URL"; then
	...
	$ISOPKG install luci-i18n-argon-config-zh-cn >/dev/null 2>&1 || true
else
	red "    GitHub 下载失败（国内常见）。两种处理方式："
	yellow "      1) 让路由器本机流量走你的代理后重跑本脚本；"
	yellow "      2) 在能访问 GitHub 的电脑上下载这两个 ipk，"
	yellow "         再用 LuCI -> 系统 -> 软件包 -> 上传安装。"
	yellow "      链接："
	yellow "        $ARGON_URL"
	yellow "        $ARGONCFG_URL"
fi
...
yellow "  - 主题来自官方 jerrykuku，iStore 来自官方 linkease"
